...
if __name__ == "__main__":
    ...
    argparser.add_argument("--no-mangling", action="store", nargs="*", default=False, metavar="foobar", help="Set to True, or a list of strings corresponding to individual variable names, to disable internal rewriting of expressions, and return same output as input expression where possible. Only applies to variables specified as first-order differential equations.")
    ...
    if isinstance(parsed_args.no_mangling, Iterable) and len(parsed_args.no_mangling) == 0:
        parsed_args.no_mangling = True
    ...
    try:
        result = odetoolbox.analysis(indict,
                                     ...
                                     no_mangling=parsed_args.no_mangling,
